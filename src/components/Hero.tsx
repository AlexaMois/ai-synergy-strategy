...
const solutions: SolutionData[] = [
  {
    ...
    problems: [
      "Заявки теряются в мессенджерах и почте — никто не знает, сколько упущено",
      "Менеджеры отвечают с задержкой — клиент забывает о вас",
      "Каждый канал требует отдельного отслеживания"
    ],
    howItWorks: [
      "ИИ-ассистент отвечает за 10 секунд, мгновенная квалификация лидов",
      "Все заявки автоматом попадают в CRM с контактами и историей",
      "Менеджер видит очередь горячих лидов и звонит только готовым к покупке"
    ],
  ...
  },
  {
    ...
    problems: [
      "Вы не видите, на каком этапе зависла заявка — узнаете вечером или на встрече",
      "Менеджеры работают по-разному — нет единого процесса",
      "Конверсия падает, но вы реагируете неделю спустя"
    ],
    howItWorks: [
      "Каждая заявка отслеживается: кому звонили, что сказано, когда следующий звонок",
      "Дашборд показывает конверсию в % — видно, кто отстает",
      "Алерты: заявка не обработана > 2 часов, и менеджер слышит уведомление"
    ],
    ...
    ctaText: "Рассчитать стоимость контроля"
  },
  {
    id: "docs",
    ...
    problems: [
      "Первичка теряется, перепутывается, бухгалтер часами ищет документы",
      "Ручной ввод данных — ошибки, которые потом сложно поймать",
      "Платеж задерживается на неделю, пока документ \"обработают\""
    ],
    howItWorks: [
      "OCR распознает текст с фото и сканов с точностью 99%",
      "Система проверит дубли, опечатки в реквизитах, соответствие договору",
      "Автоматически создает счет, запись в 1С, отправляет клиенту"
    ],
    ...
    ctaText: "Рассчитать стоимость автоматизации",
  ...
  },
  {
    ...
    problems: [
      "Новый менеджер учится 3-4 недели и ещё совершает ошибки",
      "Знания в голове одного \"старого\" сотрудника — если он уходит, теряется весь опыт",
      "Когда нужно срочно найти шаблон письма, ищут полчаса в переписке"
    ],
    howItWorks: [
      "Структурированная база: процессы, шаблоны писем, видео, чек-листы, FAQ",
      "Поиск по ключевым словам — находится за 30 секунд",
      "Каждый сотрудник видит только то, что нужно его роли"
    ],
  ...
  },
  {
    ...
    problems: [
      "Клиенты кажутся все одинаковыми — нет персонализации в подходе",
      "Отчеты объемные, но они не дают действий — вы не видите \"что делать\"",
      "Одна стратегия для всех — многие клиенты не покупают, потому что подход неправильный"
    ],
    howItWorks: [
      "ИИ выявляет паттерны: когда клиент покупает, на какую цену реагирует, что его держит",
      "Система говорит: \"Этому клиенту предложить скидку 15%\" или \"Ему нужна гарантия\"",
      "Результат: предложение подходит под клиента, а не наоборот"
    ],
    ...
    ctaText: "Рассчитать стоимость аналитики"
  },
  {
    id: "forecast",
    ...
    problems: [
      "Закупаете вслепую — берёте \"как в прошлый раз\" и попадаете в кассу",
      "То переизбыток товара, то дефицит и потеря сделок",
      "Деньги замораживаются в складе, оборот падает"
    ],
    howItWorks: [
      "ИИ анализирует историю продаж: какие месяцы горячие, какие холодные",
      "Учитывает праздники, тренды, сезонность — делает прогноз на 2-3 месяца",
      "Вы закупаете на основе данных, а не интуиции"
    ],
    ...
    ctaText: "Рассчитать стоимость прогноза"
  },
  {
    id: "tender",
    ...
    problems: [
      "Тендеры ловите вручную — может упустить важный",
      "КП готовится 2-3 дня — опаздываете на дедлайн",
      "За месяц участвуете в 5 тендерах вместо 25"
    ],
    howItWorks: [
      "Бот отслеживает все источники: Telegram-каналы, email рассылки, сайты закупок",
      "Парсит требования автоматически — очищает от лишнего",
      "ИИ генерирует КП под ваш шаблон в нужный формат"
    ],
  ...
  },
  {
    ...
    problems: [
      "Управляете по отчетам, когда уже поздно — данные запоздалые на день",
      "Невозможно быстро добавить заявку, если находитесь в пути",
      "Все процессы требуют компьютера и планов на парочку часов"
    ],
    howItWorks: [
      "ИИ понимает голосовые команды — спросить метрики или дать команду",
      "Система подключена к CRM, 1С, аналитике — получает данные в реальном времени",
      "Команды выполняются автоматически: добавить заявку, переместить в статус, отправить смс"
    ],
    ...
    ctaText: "Рассчитать стоимость решения"
  },
  {
    id: "dashboard",
    ...
    problems: [
      "Отчеты приходят вечером или на день позже — управляете постфактум",
      "Данные в разных системах — полчаса на то, чтобы собрать картину",
      "Совещания тратят час на то, чтобы разобраться в цифрах"
    ],
    howItWorks: [
      "Подключаются источники: банк, CRM, 1С, учет — все в одном месте",
      "ИИ агрегирует данные и выделяет проблемы (тревоги при отклонениях)",
      "Мобильная версия — смотрите в метро, дома, в командировке"
    ],
    ...
    ctaText: "Рассчитать стоимость дашборда"
  }
];
