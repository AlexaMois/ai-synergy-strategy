...
const publications: Publication[] = [
  ...
  { id: 24, source: 'Деловой Квартал', title: 'Новости бизнеса Красноярска', url: 'https://krasnoyarsk.dk.ru/news/237229366', type: 'read', logo: logoDelovoyKvartal, alt: 'Новости бизнеса', ariaLabel: 'Открыть новости бизнеса' },
  { id: 25, source: 'Деловой Квартал', title: 'Публикация о развитии ИИ в регионе', url: 'https://krasnoyarsk.dk.ru/news/237222687', type: 'read', logo: logoDelovoyKvartal, alt: 'Публикация о развитии ИИ', ariaLabel: 'Открыть публикацию о развитии ИИ' },
  { id: 26, source: 'Воркшоп', title: 'Воркшоп по нейросетям', url: '#', type: 'read', logo: null, alt: 'Воркшоп по нейросетям', ariaLabel: 'Узнать о воркшопе' },
];

// Первый ряд - первые 13 публикаций (разнообразные)
const firstRow = publications.slice(0, 13);
// Второй ряд - остальные
const secondRow = publications.slice(13);
...
const PublicationCard = ({ source, title, url, type, logo, alt, ariaLabel }: Omit<Publication, 'id'>) => (
  ...
  >
    ...
    <div className="flex items-center gap-1 text-primary text-sm font-medium group-hover:underline" aria-hidden="true">
...
const MarqueeRow = ({ items, direction, speed = 40 }: MarqueeRowProps) => {
  ...
  const animationClass = direction === 'left' ? 'marquee-publications' : 'marquee-publications-reverse';
  ...
  return (
    <div className="relative group">
      ...
      <div 
        ref={scrollRef}
        className={`flex gap-4 overflow-x-auto scrollbar-hide cursor-grab active:cursor-grabbing touch-pan-x py-2 ${animationClass}`}
